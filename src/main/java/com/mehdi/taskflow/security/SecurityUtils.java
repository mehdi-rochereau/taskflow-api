 ...
 *
 * <p>Reads the username from the {@link SecurityContextHolder} — populated by {@link JwtFilter}
 * after successful JWT validation — and loads the corresponding {@link User} from the database.
 *
 * <p>Centralizes authentication context access to avoid duplication across service classes and
 * simplify unit testing via mocking.
 *
 * <p>Usage example:
 *
 * <pre>{@code
...
public class SecurityUtils {
     ...
     *
     * <p>Extracts the username from the active {@link
     * org.springframework.security.core.Authentication} in the {@link SecurityContextHolder}, then
     * queries the database for the matching user.
     *
     * @return the authenticated user
     * @throws ResourceNotFoundException if no user matches the authenticated username — this should
     *     not occur in normal operation as the JWT filter already validates the token against an
     *     existing user
