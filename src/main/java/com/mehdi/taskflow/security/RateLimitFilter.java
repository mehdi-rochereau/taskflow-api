 ...
 *
 * <p>Protects against brute force attacks by limiting the number of requests per IP address on
 * sensitive authentication endpoints:
 *
 * <ul>
 *   <li>{@code POST /api/auth/login} — maximum 5 attempts per minute
 *   <li>{@code POST /api/auth/refresh} — maximum 20 attempts per minute
 *   <li>{@code POST /api/auth/register} — maximum 3 attempts per hour
 * </ul>
 *
 * <p>Uses the Token Bucket algorithm via the Bucket4j library. Each IP address gets its own bucket,
 * stored in a {@link ConcurrentHashMap}. When the bucket is empty, the request is rejected with a
 * {@code 429 Too Many Requests} response.
 *
 * <p>The IP address is extracted from the {@code X-Forwarded-For} header when available (reverse
 * proxy scenario), falling back to {@link HttpServletRequest#getRemoteAddr()}.
 *
...
public class RateLimitFilter extends OncePerRequestFilter {
     ...
     *
     * <p>Requests to other endpoints pass through without rate limiting.
     *
    ...
    /**
     * Creates a bucket allowing 5 requests per minute. Used for login endpoint protection.
     *
    ...
    /**
     * Creates a bucket allowing 20 requests per minute. Used for refresh endpoint protection.
     *
     * @return a configured {@link Bucket}
     */
    ...
    /**
     * Creates a bucket allowing 3 requests per hour. Used for registration endpoint protection.
     *
     * @return a configured {@link Bucket}
     */
     ...
     *
     * <p>Checks the {@code X-Forwarded-For} header first to handle reverse proxy scenarios (Nginx,
     * load balancer). Falls back to {@link HttpServletRequest#getRemoteAddr()} if the header is
     * absent.
     *
     * @param request the incoming HTTP request
     ...
     */
    private void rejectRequest(HttpServletResponse response, HttpServletRequest request)
            throws IOException {
