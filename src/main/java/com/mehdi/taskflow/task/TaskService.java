 ...
 *
 * <p>All operations are restricted to authenticated users. Task mutations (create, update, delete)
 * require the current user to be the owner of the associated project.
 *
 * <p>Tasks can optionally be assigned to any registered user via {@code assigneeId}. If no assignee
 * is specified, the task remains unassigned.
 *
...
public class TaskService {
     ...
     *
     * <p>Access is restricted to the owner of the project the task belongs to.
     *
     ...
     *
     * <p>Only the project owner can create tasks. If an {@code assigneeId} is provided, the
     * assignee must exist in the database.
     *
    ...
    public Task createTask(Long projectId, TaskRequest request) {
        ...
        Task task = new Task();
        task.setTitle(
                sanitizationService.sanitizeAndLog(request.getTitle(), "title", auditService));
        task.setDescription(
     ...
     *
     * <p>Ownership is verified before loading the task — if the current user is not the project
     * owner, the task is never fetched.
     *
     * <p>Input fields are sanitized before persistence to prevent XSS attacks.
     *
