 ...
 * re-authenticate. Each refresh token is single-use — upon use, it is revoked and a new one is
 * issued (rotation).
 *
 * <p>Refresh tokens are invalidated upon logout or when the user account is deleted — enforced by
 * {@code ON DELETE CASCADE} on the foreign key.
 *
 * @see User
...
public class RefreshToken {
     ...
     *
     * <p>A token is revoked after use (rotation) or upon logout. Revoked tokens are kept in the
    ...
    /**
     * Timestamp of refresh token creation. Set automatically on first persist. Cannot be updated
     * after creation.
     */
