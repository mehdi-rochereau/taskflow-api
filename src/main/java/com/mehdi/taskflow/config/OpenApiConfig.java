 ...
 *
 * <p>Configures the Swagger UI metadata via {@link OpenAPIDefinition}. No security scheme is
 * declared: authentication travels in the {@code jwt} HttpOnly cookie, which the browser attaches
 * on its own, so there is nothing for the reader to paste into an {@code Authorize} dialog.
 *
 * <p>The Swagger UI is accessible at {@code /swagger-ui/index.html}, served from {@code
 * http://localhost:8082} locally and from {@code https://api.taskflow.mehdi-rochereau.dev} in
 * production.
 *
 * <p>The API documentation (Redoc) is served by the Angular frontend at {@code /api-docs}: {@code
 * http://localhost:4200} locally, {@code https://taskflow.mehdi-rochereau.dev} in production.
 *
...
@OpenAPIDefinition(
        ...
        servers = {
            ...
            @Server(
                    url = "https://api.taskflow.mehdi-rochereau.dev",
                        ...
                        new Components()
                                .addParameters(
                                        ...
                                        new Parameter()
                                                ...
                                                .description(
                                                        "Language for error messages. Supported values: `en` (default), `fr`")
                                                .required(false)
                                                .schema(
                                                        new StringSchema()
                                                                .addEnumItem("en")
