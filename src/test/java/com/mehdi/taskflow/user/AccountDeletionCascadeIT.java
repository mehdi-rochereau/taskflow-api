...
/**
 * Verifies the referential integrity rules enforced by migration V5 on user
 * deletion.
 *
 * <p>These rules live in the database schema, not in Java: no {@code @OneToMany}
 * association is declared on {@link User}, so nothing in the persistence layer
 * compensates for a missing {@code ON DELETE} clause. Only a real engine can
 * prove the behaviour, which is why this test runs against a MySQL container
 * rather than a mock.</p>
 *
 * <p>Two distinct rules are covered: owned projects and their tasks are deleted
 * with their owner, while tasks merely assigned to the deleted user survive
 * inside the project they belong to.</p>
 */
@DataJpaTest
...
class AccountDeletionCascadeIT extends AbstractIntegrationTest {
    ...
    /**
     * Persists a user with the minimum set of non-null fields required by the
     * schema. The creation timestamp is set by the {@code @PrePersist} callback
     * on {@link User}, so it is deliberately left alone here.
     *
     ...
     *
     * <p>A JPA {@code find} would be answered from the first-level cache and
     * would report entities that the database has already cascaded away, since
     * Hibernate is unaware of engine-level deletions. A native query always
    ...
    @Test
    @DisplayName("deleting an assignee keeps tasks owned by a third party project and unassigns them")
